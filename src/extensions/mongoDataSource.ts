...
import { isArray, filter, flatten, get, head, keys, last, map, merge, includes, some, orderBy, uniq, unzip, uniqBy, isFunction } from 'lodash';
import type { Document, FilterQuery, Model, Query, UpdateQuery } from 'mongoose';
import { Types } from 'mongoose';
import type { Filter, QueryFieldsType, Sorter, SortDirection } from '@schamane/graphql-defs';
import type { DefaultGraphqlContext } from '../grapqhl/defaultGraphqlContext';
import { filterToQuery, filtersToQuery, translateSorter, languageFilter, translationsFieldPath, translationsFieldStringPath, serialExec } from '../utils';
...
export abstract class MongoDataSource<T extends Document, TContext extends DefaultGraphqlContext = DefaultGraphqlContext> extends DataSource<TContext> {
  ...
  protected async count(filters: Filter | Filter[], distinct?: string): Promise<number> {
    ...
    return distinct ? entity.distinct(distinct, await this.entityPreQuery(query)).countDocuments() : entity.countDocuments(await this.entityPreQuery(query));
  }
  ...
  private static async sortAndExecuteQueryOnDB<V extends Document>(query: Query<V[], V>, sortObject: Record<string, SortDirection>, distinct?: string): Promise<V[]> {
    return distinct ? query.distinct(distinct).sort(sortObject).exec() : query.sort(sortObject).exec();
  }

  private static async sortAndExecuteQueryOnApi<V extends Document>(query: Query<V[], V>, sortObject: Record<string, SortDirection>, distinct?: string): Promise<V[]> {
